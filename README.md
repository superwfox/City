...
### 5. 城市维度生物数量限制
- config.yml 中的 `城市维度.刷怪上限` 控制 City-World 同时存在的生物总数（包括自然刷怪与刷怪笼）。
...
指令,参数,功能说明,权限要求
/city save,无,将玩家当前所在区块标记为安全区（每日重置后保留）,OP
/city cancel,无,取消当前所在区块的安全区保护,OP
/city rewards,[页码，可选],打开全局奖励池编辑 GUI（54 格分页）,OP
/city add,无（需对准方块）,将对准的方块注册为奖励箱（自动变为箱子）,OP
/city remove,无（需对准奖励箱）,删除对准的奖励箱,OP
/city allchest,无,列出所有已注册奖励箱坐标并提供点击传送,OP
/city check,无,手动进入城市维度（坐标映射至模板世界）,OP
/city back,无,返回上次进入城市维度前的位置,OP
/city reload,无,手动触发世界完整重置 + 重载配置 + 清空已开箱记录,OP

## 配置文件（plugins/City/config.yml）
```yaml
奖励箱概率.概率值: 500        # 1-1000，数值越大每个格子出物品概率越高（500 ≈ 50%）
城市维度.刷怪上限: 500       # 城市世界最大生物数量，建议 300-800
```
